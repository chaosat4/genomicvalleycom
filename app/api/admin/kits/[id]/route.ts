...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!isValidObjectId(id))
      return withCors(request, NextResponse.json({ success: false, error: 'Invalid kit id' }, { status: 400 }));
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await connectDB();
    const kit = await Kit.findByIdAndUpdate(id, parsed.data, { new: true, runValidators: true });
    if (!kit)
      return withCors(request, NextResponse.json({ success: false, error: 'Kit not found' }, { status: 404 }));
